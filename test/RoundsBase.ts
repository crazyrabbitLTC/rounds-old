...
describe("RoundsBase Contract", function () {
  async function deployRoundBaseFixture() {
    ...
    const RoundsBaseFactory = await ethers.getContractFactory(
      "RoundsBase",
      deployer
    );
  ...
  it("should deploy and initialize the contract with correct settings", async () => {
    ...
    expect(parseInt(contractSettings.houseSplit.toString())).to.equal(
      settings.houseSplit
    );
    expect(parseInt(contractSettings.winnerSplit.toString())).to.equal(
      settings.winnerSplit
    );
    expect(parseInt(contractSettings.roundDuration.toString())).to.equal(
      settings.roundDuration
    );
    expect(parseInt(contractSettings.rounds.toString())).to.equal(
      settings.rounds
    );
    expect(parseInt(contractSettings.maxRecipientsPerVote.toString())).to.equal(
      settings.maxRecipientsPerVote
    );
    expect(contractSettings.allowPublicStartAndEnd).to.equal(
  ...
  describe("Registration", function () {
    it("should allow users to register when registration is open", async function () {
      const { roundsBase, settings, user } = await loadFixture(
        deployRoundBaseFixture
      );
    ...
    it("should revert registration when registration is closed", async function () {
      const { roundsBase, settings, user, admin } = await loadFixture(
        deployRoundBaseFixture
      );

      // Start round (and close registration)
      await expect(roundsBase.connect(admin).startNextRound()).to.emit(
        roundsBase,
        "RoundStarted"
      );
      ...
      await expect(
        roundsBase.connect(user).register()
  ...
  describe("Starting and Ending Rounds", function () {
    ...
    it("should revert starting the next round if previous round has not finished", async function () {
      ...
      await expect(
        roundsBase.connect(admin).startNextRound()
  ...
  describe("Count Participants Points", function () {
    it("should allocate points on user registration", async function () {
      ...
      const points = await roundsBase.getParticipantPoints(user.address);
      expect(points).to.equal(await roundsBase.defaultPoints()); // Assuming defaultPoints is public
    });

    it("should allocate points on starting a round", async function () {
      const { roundsBase, admin, settings } = await loadFixture(
        deployRoundBaseFixture
      );

      await roundsBase.connect(admin).startNextRound();

      const points = await roundsBase.getParticipantPoints(admin.address);
      expect(points).to.equal(await roundsBase.defaultPoints());
    });

    it("should allocate points on casting a vote", async function () {
      const { roundsBase, user, admin, recipients } = await loadFixture(
        deployRoundBaseFixture
      );
      ...
      const points = await roundsBase.getParticipantPoints(user.address);
      expect(points).to.equal(await roundsBase.defaultPoints() * BigInt(2)); // Points for registration and voting
    });

    it("should correctly accumulate points over multiple actions", async function () {
      const { roundsBase, user, admin, recipients, settings } =
        await loadFixture(deployRoundBaseFixture);

      await roundsBase.connect(user).register();
      await roundsBase.connect(user).startNextRound();
      await roundsBase.connect(user).castVote([recipients[0].address]);

      const points = await roundsBase.getParticipantPoints(user.address);
      expect(points).to.equal(await roundsBase.defaultPoints() * BigInt(3)); // Points for registration, starting round, and voting
    });
  });
  ...
  describe("Tally Total Votes", function () {
    it("should correctly tally votes for a single round", async function () {
      ...
      const votes = await roundsBase.getCandidateTotalVotes(
    ...
    it("should show zero votes when no votes are cast", async function () {
      const { roundsBase, admin, recipients, settings } = await loadFixture(
        deployRoundBaseFixture
      );
    ...
    it("should correctly tally votes for multiple candidates", async function () {
      ...
      await roundsBase
        .connect(user)
        .castVote([recipients[0].address, recipients[1].address]);
      await time.increase(settings.roundDuration + 1);

      const votesRecipient1 = await roundsBase.getCandidateTotalVotes(
        recipients[0].address
      );
      const votesRecipient2 = await roundsBase.getCandidateTotalVotes(
    ...
    it("should handle max votes per user per round", async function () {
      ...
      const maxVotesArray = Array(settings.maxRecipientsPerVote).fill(
        recipients[0].address
      );
    ...
    it("should correctly tally votes across rounds with different voting patterns", async function () {
      ...
      const votesRecipient0 = await roundsBase.getCandidateTotalVotes(
        recipients[0].address
      );
      const votesRecipient1 = await roundsBase.getCandidateTotalVotes(
        recipients[1].address
  ...
  describe("Registration", function () {
    it("should allow a user to register before any rounds", async function () {
      const { roundsBase, settings, user, admin, recipients } =
        await loadFixture(deployRoundBaseFixture);
      await expect(roundsBase.connect(user).register()).to.emit(
        roundsBase,
        "UserRegistered"
      );
    });
  ...
  describe("Cast Vote", function () {
    it("should not allow a user to cast a vote with no rounds", async function () {
      ...
      await roundsBase.connect(user).register();
      await expect(
        roundsBase.connect(user).castVote(recipients)
    ...
    it("should revert if too many votes are cast", async function () {
      ...
      const tooManyRecipients = new Array(
        settings.maxRecipientsPerVote + 1
      ).fill(user.address);
      await expect(
        roundsBase.connect(user).castVote(tooManyRecipients)
    ...
    it("should correctly increment the number of votes cast in the current round", async function () {
      ...
      const correctNumberOfRecipients = new Array(
        settings.maxRecipientsPerVote
      ).fill(user.address);
      await roundsBase.connect(user).register();
      await roundsBase.connect(admin).startNextRound();
      ...
      const votesCast = await roundsBase.numberOfVotesCastInThisRound(
        user.address,
        currentRound
      );
    ...
    it("should revert if trying to cast a vote in a round that has not started", async function () {
      const { roundsBase, user, recipients } = await loadFixture(
        deployRoundBaseFixture
      );
    ...
    it("should allow user to vote more than once in the same round if they still have votes", async function () {
      ...
      await expect(
        roundsBase.connect(user).castVote(correctNumberOfRecipients)
      ).to.emit(roundsBase, "VoteCast");
      await expect(
        roundsBase.connect(user).castVote(correctNumberOfRecipients)
      ).to.emit(roundsBase, "VoteCast");
    ...
    it("should not allow user to vote more than once in the same round if they run out of  votes", async function () {
      const { roundsBase, user, admin } = await loadFixture(
        deployRoundBaseFixture
      );
      ...
      await expect(roundsBase.connect(user).castVote(recipients)).to.emit(
        roundsBase,
        "VoteCast"
      );
      await expect(roundsBase.connect(user).castVote(recipients)).to.emit(
    ...
    it("should allow user to vote more than once in the same round if they still have votes", async function () {
      ...
      await expect(roundsBase.connect(user).castVote(recipient)).to.emit(
        roundsBase,
        "VoteCast"
      );
      await expect(roundsBase.connect(user).castVote(recipient)).to.emit(
    ...
    it("should correctly report the number of votes for an address", async function () {
      const { roundsBase, admin, user, recipients } = await loadFixture(
        deployRoundBaseFixture
      );
      ...
      const voteOfaNoncandidate = await roundsBase.getVotes(
        admin.address,
        currentRound
      );
      expect(votes).to.equal(1);
  ...
  describe("Vote Integrity", function () {
    it("should correctly tally votes", async function () {
      ...
      const votesForFirstRecipient = await roundsBase.getVotes(
        recipients[0].address,
        currentRound
      );
      const votesForSecondRecipient = await roundsBase.getVotes(
        recipients[1].address,
        currentRound
      );
...
describe("Elimination Logic", function () {
  ...
  it("should correctly eliminate candidates based on voting", async function () {
    const { roundsBase, admin, user, user2, user3, user4, settings } =
      await loadFixture(deployRoundBaseFixture);
